...
esp_err_t i2c_bus_read_reg(i2c_bus_id_t bus, uint8_t dev_addr, uint8_t reg, uint8_t *value)
{
    ...
    ret = i2c_master_transmit_receive( dev_handle, &reg_buf, 1, value, 1, 100);
...
esp_err_t i2c_bus_read_regs(i2c_bus_id_t bus, uint8_t dev_addr, uint8_t reg, uint8_t *data, size_t len)
{
    if ((data == NULL) || (len == 0)) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    ret = i2c_master_transmit_receive(dev_handle, &reg, 1, data, len, 100);
